...
public class TutorialScript : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		WriteMessage("Welcome to Judgement Day!",0.75f,0.5f,6f);
	...
	void Update () 
	{
		...
		{
			WriteMessage("You are a GOD!!!",0.75f,0.5f,6f);
			WriteMessage("but these people are not adoring you",0.85f,0.5f,6f);
			messageCount++;
		}
		...
		{
			WriteMessage("Why not try a little FIRE!!",0.75f,0.5f,5f);
		...
		{
			WriteMessage("JAJAJAJAJAJAJAJAJA...",0.75f,0.5f,4f);
			messageCount++;
		}
		...
		{
			WriteMessage("You missed. Did you not?",0.75f,0.5f,6f);
			messageCount = 9;
		}
		
		
		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==9 && GlobalManager.globalManager.population < 2){
			WriteMessage("Sorry, I got carried away",0.75f,0.5f,6f);
			messageCount++;
		}
		...
		{
			WriteMessage("Do not worry. You can Redo the level any time",0.75f,0.5f,6f);
			WriteMessage("Press the Redo button up in the left",0.75f,0.5f,0f);
			messageCount+=10;
		}
		...
		{
			((BasePowerDealer)firePower).enableUse = false;
			WriteMessage("However be aware! When you use your powers you use SOULS",0.75f,0.5f,8f);
			WriteMessage("Your number of SOULS is up there with the crosses",0.85f,0.5f,8f);
			messageCount++;
		}

		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==11){
			WriteMessage("You have a limited number of SOULS",0.75f,0.5f,8f);
			WriteMessage("But with each infidel dead you get his sins weight in SOULS",0.85f,0.5f,8f);
			messageCount++;
		}

		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==12)
		{
			WriteMessage("Some will be worth more of your time, some less",0.75f,0.5f,8f);
			WriteMessage("It depends on their actions, and Yours",0.85f,0.5f,8f);
			WriteMessage("This guys will give you 3 souls each",0.95f,0.5f,8f);
			messageCount++;
		}

		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==13){
			WriteMessage("The cost of the Powers is stated next to it",0.75f,0.5f,8f);
			WriteMessage("You can use some extra SOULS this time",0.85f,0.5f,8f);
		...
		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==14)
		{
			...
			{
				((BasePowerDealer)firePower).enableUse = true;
				WriteMessage("Now, Do me a favor and kill that other infidel too.",0.75f,0.5f,7f);
				messageCount++;
			}
			else
			{
				WriteMessage("Nice!! you got 2 for 1",0.75f,0.5f,8f);
				messageCount+=2;
			}
		}
		...
		{
			WriteMessage("Good Job!!!",0.75f,0.5f,4f);
			messageCount++;
		}
		...
		{
			WriteMessage("Looks like you are out of souls. Do not worry, you can Redo the level any time",0.75f,0.5f,7f);
			WriteMessage("Just press the Redo button up in the left",0.75f,0.5f,0f);
		}

		if(!LevelGUI.levelGUI.isDisplayingMessages() && messageCount==16 && GlobalManager.globalManager.population <= 0)
		{
			WriteMessage("You managed to kill all the infidels. You are a worthy god",0.75f,0.5f,10f);
			WriteMessage("Now, let's go kill some more people",0.85f,0.5f,10f);
			messageCount++;
		}
