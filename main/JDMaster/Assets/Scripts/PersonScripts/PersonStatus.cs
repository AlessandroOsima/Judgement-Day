...
public class PersonStatus : MonoBehaviour
{
    ...
    //Sounds
    public AudioClip talkingtClip;
    public AudioClip laughingClip;
    public AudioClip screamingClip2;
        ...
        set
        {
            ...
            if (value == Status.Dead && unitStatus != Status.Dead)
            {
                if (ShouldNotBeKilled)
                {
                    ...
                    LevelGUI.levelGUI.writeMessage("-" + (-2 * soulPoints), new Vector3(1.85f * (Screen.width / 2), Screen.height / 10, 0f), new Vector3(1.4f, 1.4f, 1), 3f, 2);
                }
                else
                {
                    GlobalManager.globalManager.incrementScore(scorePoints);
                    ...
                    GlobalManager.globalManager.decrementPopulation(1);
                    LevelGUI.levelGUI.writeMessage("x" + soulPoints, new Vector3(1.85f * (Screen.width / 2), Screen.height / 10, 0f), new Vector3(1.4f, 1.4f, 1), 3f, 2);
                }

                if (audio.isPlaying && audio.clip != dyingClip)
    ...
    public int Fear
    {
        ...
        set
        {
            ...
            if (value > 70)
            {
                if (audio.isPlaying && audio.clip != screamingClip)
                {                   // if there is audio playing
                    audio.Stop();
