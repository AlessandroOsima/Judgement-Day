...
public class LevelGUI : MonoBehaviour
{
    ...
    public void writeMessage(string message, Vector3 position, Vector3 scale, float duration, int color, bool useBackground = true)
    {
        if (messages == null)
        {
            messages = new List<TextMessage>();
        ...
        if (color == 2)
        {
            textMessage.text.setColorForAllLetters(sGreen);
        }
