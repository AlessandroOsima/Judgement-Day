...
public class VertexPainter2 : EditorWindow {
		...
		if (showAdvanced) {
			...
			if (lockedObject) {
				...
				if (GUILayout.Button("  See Vertex Colors ",GUILayout.Height(25))) {
					...
					if (lockedObject.renderer) {

						if (lockedObject.renderer.sharedMaterial.shader.name != "vertexPainter/VertexColorsOnly") {

							lockedObject.renderer.sharedMaterial.shader = Shader.Find("vertexPainter/VertexColorsOnly");
	...
	private void colorsTo(bool toMobile) {
		...
		Material selectedMaterial = lockedObject.renderer.sharedMaterial;
		...
		object[] obj = GameObject.FindObjectsOfType(typeof(GameObject));
		  ...
		  foreach (GameObject o in obj)
		  {
			  ...
			  Material[] mats = o.renderer.sharedMaterials;
	...
	private void setupShader() {
		...
		if (lockedObject.renderer.sharedMaterial) {

			shaderName = lockedObject.renderer.sharedMaterial.shader.name;
	...
	private void getShader() {
		...
		if (lockedObject.GetComponent<Renderer>().sharedMaterial) {
			...
			lockedObject.renderer.sharedMaterial = theMaterial;
			shaderName = "Diffuse";
	...
	private void addUpdateShader(string shaderType, bool isExisting) {
		...
		Material theMaterial = lockedObject.renderer.sharedMaterial;
